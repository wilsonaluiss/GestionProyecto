...
public class ConsultaReserva {
    ...
    public ObservableList <ReservaRecurso>buscarTodos(){
        String query = "select * from reserva";
        ConsultasDb jdbc = new ConsultasDb();
        ResultSet rs = jdbc.realizarConsulta(query);
        ObservableList<ReservaRecurso> reservaRecurso = FXCollections.observableArrayList();
        
        try {
            while(rs.next()){
                int codigoReserva = rs.getInt("codigo_reserva");
                String nombreActividad  = rs.getString("nombre_actividad");
                String descripcion  = rs.getString("descripcion_actividad");
                Date fecha  = rs.getDate("fecha");
                String horaInicio = rs.getString("hora_inicio");
                String horaFin  = rs.getString("hora_fin");
                reservaRecurso.add(new ReservaRecurso(codigoReserva, nombreActividad, descripcion, fecha, horaInicio, horaFin));
            }
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(null, "Error al buscar en la reserva de recurso : "+ex,
                    "Error",JOptionPane.WARNING_MESSAGE);
        }
